...
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
import { Users, FileText, Clock, CheckCircle, AlertTriangle, Plus, Upload } from "lucide-react"
import { useNavigate } from "react-router-dom"
import { useEffect, useState } from "react"
import { getMyClients, getMyApplications, type Client, type Application } from "../../lib/employee"
import PaymentReceiptUpload from "@/components/PaymentReceiptUpload" // ✅ Import new component
  ...
  return (
    <DashboardLayout userRole="employee" userName="Sarah Johnson">
      <div className="space-y-4 sm:space-y-6 p-4 sm:p-6">
        ...
        <Card className="rounded-3xl border-0 bg-white/80 backdrop-blur-sm shadow-lg hover:shadow-xl transition-all duration-300">
          <CardHeader className="pb-4">
            <CardTitle className="text-lg font-semibold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent flex items-center gap-2">
              <Upload className="h-5 w-5 text-blue-600" />
              Payment Receipt Upload
            </CardTitle>
          </CardHeader>
          <CardContent>
            <PaymentReceiptUpload userId={"employee123"} role="employee" /> {/* replace with real employee._id */}
          </CardContent>
        </Card>
